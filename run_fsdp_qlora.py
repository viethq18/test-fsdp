...
class ScriptArguments:
    train_dataset_path: str = field(
        default=None,
        metadata={"help": "Path to the dataset, e.g. /workspace/code/code2/train/"},
    )
    test_dataset_path: str = field(
        default=None,
        metadata={"help": "Path to the dataset, e.g. /workspace/code/code2/test/"},
    )
    model_id: str = field(
        default=None, metadata={"help": "Model ID to use for SFT training"}
    )
    max_seq_length: int = field(
        default=4096, metadata={"help": "The maximum sequence length for SFT Trainer"}
    )
...
def training_function(script_args, training_args):
    ...
    end_time = time.time()
    print(f"TRAINING EXECUTION TOOK: {end_time - start_time} seconds")
